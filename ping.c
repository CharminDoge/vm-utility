// credits to https://www.geeksforgeeks.org/ping-in-c/
...
// make a ping request
long double send_ping(int ping_sockfd, struct sockaddr_in *ping_addr,
				char *ping_dom, char *ping_ip, char *rev_host, int cycle)
{
	int internal_cycle = cycle;
	...
	while(pingloop)
	{
		...
		// break statement
		if (cycle != 0 && --internal_cycle == 0) {break;}
	...
	if (msg_count != msg_received_count){
		average /= msg_received_count;
	}
...
// Driver Code
long double main_send_ping(char *ip, int cycles)
{
	int sockfd;
	...
	ping_rtt = send_ping(sockfd, &addr_con, reverse_hostname,
								ip_addr, ip, cycles);
